...
//     비용 2 방어(S02)가 끼면 승률이 51.2% → 15.2% 로 무너진다. 필력 3/턴에서 비용 2 방어는
//     그 턴의 공격을 통째로 먹는다. 실측 감도: 방어 1장 69.0% / 2장 51.2% / 3장 33.0% —
//     **과투자가 처벌되는 구조**이며 40~60% 목표대는 2장 근방이다.
// boss1_def = 방어 특화 대조군 19장 (상점·이벤트를 전부 찬양에 쓴 경우) — 방어 축 감도 측정용.
const DECK_PRESETS: Record<string, (startingDeck: string[]) => string[]> = {
  boss1: (s) => [...s, 'G01', 'G02', 'D02', 'X02', 'H02', 'H04'],
